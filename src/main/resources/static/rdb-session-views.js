...
(function () {
    ...
    var CSS = [
        ...
        '.rsv-count { color:var(--text-muted); font-size:13px; }',
        '.rsv-count b { color:var(--text); font-weight:600; }',
        '.rsv-note { color:var(--text-muted); font-size:13px; margin:0 0 12px; line-height:1.65; max-width:940px; }',
        '.rsv-spacer { flex:1; }',
        '.rsv-selected { color:var(--text-muted); font-size:13px; }',
        ...
        /* 줄 수가 많은 표에서 눈이 행을 놓치지 않도록 옅은 줄무늬를 깐다. hover 강조(clickable)보다
           연해서 "지금 가리키는 행" 표시를 덮지 않는다. */
        '.rsv-table tbody tr:nth-child(even) { background:rgba(255,255,255,.028); }',
        ...
        '.rsv-empty, .rsv-error { padding:18px; text-align:center; font-size:13.5px; }',
        ...
        '.rsv-badge { display:inline-block; font-size:10.5px; font-weight:700; letter-spacing:.3px;',
        ...
        '.rsv-cycle { color:#ef4444; font-size:12px; margin-left:6px; }',
        ...
        '.rsv-modal-head h3 { margin:0; font-size:15.5px; color:var(--text); font-weight:600; }',
        ...
        '.rsv-warn { color:var(--orange); font-size:13.5px; line-height:1.6; margin:0 0 12px; }',
        '.rsv-idlist { margin:0; padding:10px 12px; background:#17181c; border:1px solid var(--border); border-radius:4px;',
        '  color:var(--text); font-family:Consolas,Menlo,monospace; font-size:13px; max-height:30vh; overflow:auto; }',
        '.rsv-result-line { font-size:13.5px; padding:5px 0; border-bottom:1px solid var(--border); }',
        ...
        '.rsv-field { font-size:13px; border-bottom:1px solid var(--border); padding-bottom:6px; }',
        ...
        '.rsv-sql-head span { color:var(--text-muted); font-size:13px; }',
        ...
        '  color:var(--text); font-family:Consolas,Menlo,monospace; font-size:13px; line-height:1.6;',
        ...
        '.rsv-lock-note { margin:0 0 8px; padding:8px 10px; border-radius:4px; font-size:13px; line-height:1.65;',
    ...
    window.dbagentInitRdbSessionViews = function (options) {
        ...
        // 엔진에서 세 탭이 통째로 죽는다</b> - 그 화면에 없는 컨테이너 하나 때문에 나머지 탭까지
        // 초기화가 멈추던 것을 2026-09-06 에 고쳤다. 화면마다 어떤 탭을 둘지는 각 HTML 의 마크업이
        // 정하고, 이 파일에는 db_type 분기를 두지 않는다.
        ...
        // 기본 문구는 holder 를 알 수 있는 엔진(MySQL/MariaDB/PostgreSQL/MS SQL) 기준이다. holder 를
        // 못 주는 엔진은 이 문구가 그대로 나가면 "트리로 보여준다" 가 화면과 어긋나고, 바로 아래
        // 서버가 준 blocker_note("holder 를 알려주지 않습니다")와도 정면으로 모순된다. 그래서 화면이
        // 자기 문구를 넘길 수 있게 열어 둔다 - 여기에 db_type 분기를 넣지 않는 것은 탭 구성을
        // 마크업이 정하는 것과 같은 이유다.
        //
        // <b>"맨 위 Holder 를 처리하면 아래가 한꺼번에 풀립니다" 라고 적혀 있었으나 사실이 아니어서
        // 2026-09-06 에 고쳤다.</b> 3단 체인(A<-B<-C)에서 A 를 kill 하면 B 만 풀리고 <b>C 는 계속
        // 대기</b>한다 - B 가 자기 락을 그대로 쥐고 있기 때문이다(실측: 61 kill 후 295 가 새 홀더가
        // 되고 308 은 그대로 대기). 오히려 중간의 BOTH 를 kill 하면 그 아래가 풀리고 맨 위 Holder 는
        // 막을 상대가 없어져 한 번에 정리된다. 다만 그때 Holder 의 트랜잭션은 살아 있어 다음 세션을
        // 또 막을 수 있으므로, 원인까지 없애려면 Holder 와 BOTH 를 함께 골라 kill 해야 한다.
        var locksPane = buildPane(locksView,
            (opts.locksNote ||
                '락을 쥔 세션(Holder)을 위에, 그 락을 기다리는 세션(Waiter)을 아래에 붙여 트리로 보여줍니다. ' +
                'A가 B를, B가 C를 막는 다단계 체인도 그대로 이어지며, 가운데 세션은 BOTH로 표시됩니다. ' +
                '맨 위 Holder만 종료하면 바로 아래 한 단계만 풀리고 그 아래는 계속 기다립니다 - ' +
                '체인을 한 번에 정리하려면 Holder와 BOTH를 함께 선택해 종료하세요. ' +
                '행을 클릭하면 SQL 전문을 볼 수 있습니다.') +
            // Kill 안내는 문구를 바꾼 화면에도 그대로 붙인다 - 세션 Kill 은 엔진과 무관하게
            // 관리자에게만 열려 있고, 이 탭의 체크박스 동작도 모든 화면에서 같다.
            (admin ? ' 왼쪽 체크박스로 세션을 골라 강제 종료할 수 있습니다.' : ''));
